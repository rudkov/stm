...
function HipsFilter(props) {
    ...
    const onChangeComplete = (values) => {
        ...
        if (!isDefault) {
            ...
            sessionStorage.setItem('talentsPage.filteredHips', JSON.stringify(values));
        }
        else {
            setIsApplied(false);
            props.setFiltered([]);
            sessionStorage.setItem('talentsPage.filteredHips', JSON.stringify([]));
        }
    };
