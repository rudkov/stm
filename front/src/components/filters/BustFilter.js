...
function BustFilter(props) {
    ...
    const onChangeComplete = (values) => {
        ...
        if (!isDefault) {
            ...
            sessionStorage.setItem('talentsPage.filteredBust', JSON.stringify(values));
        }
        else {
            setIsApplied(false);
            props.setFiltered([]);
            sessionStorage.setItem('talentsPage.filteredBust', JSON.stringify([]));
        }
    };
